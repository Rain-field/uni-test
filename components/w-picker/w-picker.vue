...
<script>
	import {initPicker,initDays} from "./date-data/util.js";
			...
			bindChange(val){
				...
				switch(mode){
					...
					case "region":
						...
						this.returnValue = this.data.provinces[arr[0]].value+","+this.data.provinces[arr[0]].value+","+this.data.areas[arr[2]].value;
						break;
				}
				this.pickVal=arr;
			},
			initData(){
				...
				switch(mode){
					...
					case "region":
						...
						this.returnValue = province.label+","+city.label+","+area.value;
						break;
				}
			}
		},
